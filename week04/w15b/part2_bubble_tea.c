...
struct bubble_tea scanning(void);
double calculating_cost(struct bubble_tea order);
...
int main(void) {
    ...
    // Scanning
    struct bubble_tea order = scanning();
    ...
    // Calculate the cost
    double total_cost = calculating_cost(order);
